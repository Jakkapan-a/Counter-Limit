...
BUTTON_STATE currenButtonState = BUTTON_STATE::NONE;
bool IsPress = false;
int countPress = 0;
...
void loop() {
  ...
  if (millis() - lastTimeMain >= 1000) {
    ...
    if (display == Settings) {
      ...
      // currenButtonState = BUTTON_STATE::ESC;
      // IsPress = true;
      // countPress = 0 ;
      if (IsPress) {
        countPress++;
  ...
  if (millis() - lastTimeBtnPress > 10) {
    // ---------------------------- BUTTON PRESS ---------------------------------- //
    if (IsPress && countPress > 1 && display == Settings && setings != Index) {
      if (currenButtonState == BUTTON_STATE::UP || currenButtonState == BUTTON_STATE::DOWN) {
        Serial.println("Press: " + String(countPress));
        BUTTON_PUSHES(currenButtonState);
      }
    }
...
void ButtonEnterPressed(void) {
  BUTTON_PUSHES(ENTER);
  currenButtonState = BUTTON_STATE::ENTER;
  IsPress = true;
  countPress = 0;
}
...
void ButtonUpPressed(void) {
  BUTTON_PUSHES(UP);
  currenButtonState = BUTTON_STATE::UP;
  IsPress = true;
  countPress = 0;
}
...
void ButtonDownPressed(void) {
  BUTTON_PUSHES(DOWN);
  currenButtonState = BUTTON_STATE::DOWN;
  IsPress = true;
  countPress = 0;
}
